"use client"; // <--- INI KUNCINYA RIS, JANGAN SAMPAI LEPAS!
...
export default function SocialShare({ shareUrl, title }: SocialShareProps) {
  ...
  const iconBoxStyle = (bgColor: string): React.CSSProperties => ({
    ...
    color: '#fff',
    textDecoration: 'none',
    border: 'none' // Tambahkan ini agar button tidak punya border default
  });
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '20px', margin: '40px 0', fontFamily: 'Arial, sans-serif' }}>
      
      {/* LABEL BAGIKAN DENGAN POINTER */}
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 20px', border: '1px solid #e2e8f0', borderRadius: '4px', backgroundColor: '#fff', minWidth: '120px' }}>
        <Share2 size={18} color="#334155" />
        <div style={{ width: '1px', height: '20px', backgroundColor: '#e2e8f0' }}></div>
        <span style={{ fontWeight: '700', color: '#334155', fontSize: '15px' }}>Bagikan</span>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        {/* WhatsApp */}
        <a href={`https://api.whatsapp.com/send?text=${encodedTitle}%20${encodedUrl}`} target="_blank" className="share-btn" style={iconBoxStyle('#7dbd6a')}>
        ...
        {/* Copy Link - INI YANG BIKIN ERROR TADI */}
        <button onClick={handleCopyLink} className="share-btn" style={iconBoxStyle('#000')}>
          <LinkIcon size={18} />
        </button>
      </div>
